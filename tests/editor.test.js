...
(async () => {
	...
	dom.window.Pagevia = { postId: 1, restPath: '/pagevia/v1/documents/', nonce: 'test', upgradeUrl: '', advancedDesign: true, widgetTypes: [['test-product', 'Test Product']], widgetDefaults: { 'test-product': { limit: 4 } }, widgetControls: { 'test-product': [{ name: 'limit', label: 'Product limit', type: 'number' }] } };
	...
	if (![...dom.window.document.querySelectorAll('.pagevia-field span')].some((node) => node.textContent === 'Entrance animation')) throw new Error('Advanced design controls missing');
